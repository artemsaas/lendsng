...
<head>
  ...
  <script>
    ...
    window.onload = function () {
      ...
      if (isMobile()) {
        let savedOffer = localStorage.getItem("userOfferIndex");

        if (savedOffer === null) {
          // Новый пользователь — выбираем оффер случайно
          const randomIndex = Math.floor(Math.random() * offers.length);
          localStorage.setItem("userOfferIndex", randomIndex);
          savedOffer = randomIndex;
        }

        // Редирект на выбранный оффер
        window.location.href = offers[savedOffer];
      } else {
        // Десктопный редирект
